...
do 
    ...
    for ((j=$i;j<=${#fnames[@]}-1;j++))
    do
	# do not re-compute cross-correlations if it is already computed
	if [ -e "/common/external/rawabd/Mindboggle_cc_details/cc_$i-$j.txt" ] ; then
	    cc=$( tail -n 1 /common/external/rawabd/Mindboggle_cc_details/cc_$i-$j.txt )
